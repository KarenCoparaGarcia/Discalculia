...
/// Este guión maneja las entradas del usuario como botones
...
public class InputsHandler : MonoBehaviour
{
		...
		private void ScreenClickHandle (GameObject ob, bool isTouchBegain)
		{
				...
				foreach (GameObject btn in buttons) {
						...
						Boton btnComp = btn.GetComponent<Boton> ();
						if (btnComp == null) {
								continue;
						}
		...
		private void ScreenClickEndedHandle ()
		{
				...
				Boton btnComp = lastClickedBtn.GetComponent<Boton> ();
				if (btnComp == null) {
						return;
				}
				...
				if (!string.IsNullOrEmpty (btnComp.message)) {
					...
					Debug.LogWarning ("mensaje vacío en <i>" + lastClickedBtn.name + "</i>click");//you missed to add event name
				}
